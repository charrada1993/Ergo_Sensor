...
def plot_learning_curves(reg_log, cls_log, sev_log):
    ...
    if reg_log:
        ...
        axes[0].set_title('Model 1: Risk Score'); axes[0].set_xlabel('Trees'); axes[0].set_ylabel('RMSE'); axes[0].legend()
    if cls_log:
        ...
        axes[1].set_title('Model 2: Condition'); axes[1].set_xlabel('Trees'); axes[1].set_ylabel('LogLoss'); axes[1].legend()
    if sev_log:
        ...
        axes[2].set_title('Model 3: Severity'); axes[2].set_xlabel('Trees'); axes[2].set_ylabel('LogLoss'); axes[2].legend()
    plt.tight_layout()
...
def plot_anomaly_curves(anom_res):
    ...
    for ax, (col, res) in zip(axes, anom_res.items()):
        log = res.get('log', [])
        if not log: continue
        ax.plot([x['tree'] for x in log], [x.get('train_binary_logloss',0) for x in log], label='Train', color='#00d4ff')
        ax.plot([x['tree'] for x in log], [x.get('valid_binary_logloss',0) for x in log], label='Valid', color='#ff00aa')
        ax.set_title(col.replace('anomaly_', '').replace('_', ' ').title(), fontsize=10)
        ax.legend()
    plt.tight_layout()
...
def main():
    print("\n" + "="*72)
    print("  ERGO SENSOR AI ENGINE v3.0-Production — OPTUNA + SHAP")
    print("="*72)
    t0 = datetime.now()
    ...
    cls_m, cls_log = train_classifier(X_tr, X_te, y_cls_tr, y_cls_te, len(meta['condition_to_code']), feats)[1:]
