"""
report_generator.py  —  v3.0-Production
========================================
Professional medical-grade PDF report for Ergo Sensor.
AI Engine: LightGBM Regressor + Condition Classifier (Optuna HPO, TimeSeriesSplit CV)
            + Severity Classifier + IsolationForest(300) + 5× Per-Joint Anomaly
            75 features (38 base + 37 engineered: rolling, lag, accel, asymmetry, energy)
            R²=0.9981 | CondAcc=99.60% | SevAcc=96.95% | Anomaly avg-F1=0.9906
...
def _draw_header_footer(canvas, doc, is_first):
    ...
    canvas.drawRightString(w - 1.5 * cm, h - 1.05 * cm,
                           'Musculoskeletal Disorder Risk Assessment  |  v3.0-Production')
    # accent stripe
    canvas.setFillColor(C_TEAL)
    ...
    canvas.drawString(1.5 * cm, 0.45 * cm,
                      f'Generated: {datetime.now().strftime("%Y-%m-%d %H:%M")}  |  AI Engine v3.0-Production (75 feat, Optuna HPO, SHAP)  |  CONFIDENTIAL')
    canvas.setFillColor(C_WHITE)
...
class ReportGenerator:
    ...
    def _section_cover(self, df, stats):
        ...
        # AI Engine info table
        story.append(Paragraph("AI ENGINE — v3.0-Production",
                               ParagraphStyle('AiHdr', parent=self._styles['subtitle'],
                                              alignment=TA_LEFT, textColor=C_GREY_MID, fontSize=9)))
        story.append(Spacer(1, 0.2 * cm))
        ai_info = [
            ['Model 1', 'LightGBM Regressor',           'Risk Score',     'R²=0.9981  RMSE=0.0079  MAE=0.0056'],
            ['Model 2', 'LightGBM Classifier (GBDT)',   '18 Conditions',  'Acc=99.60%  Recall=95.13%  F1=96.67%'],
            ['Model 3', 'LightGBM Severity Classifier', '3 Levels',       'Acc=96.95%  Macro-F1=94.11%'],
            ['Model 4', '5× Per-Joint Anomaly',         'Binary Detect',  'avg F1=99.06%  Trunk F1=100%'],
            ['Model 5', 'IsolationForest (300 trees)',   'Global Anomaly', '5% contamination  max_samples=256'],
        ]
        ai_hdr = [['#', 'Model', 'Output', 'Performance']]
        ai_t = Table(ai_hdr + ai_info, colWidths=[1*cm, 4.8*cm, 3.5*cm, 5.7*cm], hAlign='LEFT')
        ai_ts = [
        ...
        ai_t.setStyle(TableStyle(ai_ts))
        story.append(ai_t)
        story.append(Spacer(1, 0.4 * cm))
        story.append(Paragraph(
            'HPO: Optuna (10 trials) + TimeSeriesSplit(3-fold) — temporal leakage-free validation. '
            'Features: 75 total (38 base biomechanical + 37 engineered: rolling mean/std 0.5s, '
            'lag-15 frames, joint acceleration, bilateral asymmetry, composite load, posture flags). '
            'SHAP TreeExplainer identifies the joint driving each prediction.',
            ParagraphStyle('AiNote', parent=self._styles['small'], textColor=C_GREY_MID)
        ))
        story.append(Spacer(1, 0.5 * cm))

        story.append(Paragraph(
            'Generated by automated kinematic monitoring. Requires medical review. '
            'Combines real-time joint angle measurements, RULA/REBA ergonomic scoring, '
            'and the Ergo Sensor AI Engine v3.0-Production (LightGBM + IsolationForest) '
            'to provide a comprehensive musculoskeletal disorder risk assessment.',
    ...
    def _section_ai_insights(self, stats, ai_df):
        ...
        story.append(Spacer(1, 0.15 * cm))
        story.append(Paragraph(
            '<b>AI Engine v3.0-Production Methodology:</b> '
            'The LightGBM Regressor (Model 1, 75 features, R²=0.9981) predicts a continuous '
            'risk score per session. Model 2 — GBDT Classifier tuned by Optuna with TimeSeriesSplit '
            '3-fold CV — classifies into 18 TMS pathologies (Acc=99.60%, F1=96.67%). '
            'Model 3 assigns severity (Low/Medium/High, Acc=96.95%). '
            'Five per-joint LightGBM binary classifiers (Model 4, avg F1=99.06%) flag granular '
            'postural anomalies. Model 5 — IsolationForest(300, contamination=5%) — '
            'identifies globally anomalous movement patterns. '
            'SHAP TreeExplainer identifies which of the 75 features (including rolling stats, '
            'lag-15, and joint accelerations) contribute most to each risk prediction.',
            S['small']
        ))
        return story
    ...
    def _section_clinical_recommendations(self, stats):
        ...
        if 'ai_anomaly' in stats and stats['ai_anomaly']['max'] >= 0.7:
            recs.append(('AI ALERT',
                         'IsolationForest (Model 5) identified critical postural anomalies '
                         '(score ≥ 0.7). Immediate technique correction and ergonomic evaluation advised.'))
        ...
        story.append(Paragraph(
            ...
            '(AI Engine v3.0-Production — LightGBM + IsolationForest, 75 features, Optuna HPO, '
            'TimeSeriesSplit cross-validation, SHAP explainability). '
            'It is intended as a clinical decision-support tool only and does not replace professional '
            'medical or occupational health evaluation. All recommendations must be reviewed '
